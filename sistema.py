...
if seleccion == "🚀 Añadir Logo":
    ...
    with st.form("vels_form_mail", clear_on_submit=False):
        ...
        with col_a:
            ...
            email_user = st.text_input("Tu Correo", value=st.session_state.email_pref, key="usr_1") 
            email_pass = st.text_input("Contraseña de Aplicación", value=st.session_state.pass_pref, type="password", key="pwd_1")
        ...
        with col_b:
            buscar_texto = st.text_input("Palabra clave a buscar (ej: DTE o Correo origen)", value="DTE", key="txt_1")
